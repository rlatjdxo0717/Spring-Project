...
<script type="text/javascript">
	function cs(cart) //cs함수사용
	{
		var sum = 0; //sum 변수 생성 초기값으로 0을 부여
		var count = cart.ck.length; // count 변수 생성 체크박스 갯수
		for (var i = 0; i < count; i++) { // 체크박스 갯수많큼 for문 돌리기
		...
		cart.total_sum.value = sum; //변경된 값을 sum으로 반환
	}
</script>
...
<body>
	<form name="form">
		여기는 장바구니 페이지...
		<hr color=blue>
		${user_Id} 님이 추가한 물품들입니다.<br> 구입하고싶은 물품은 체크 하시고 결제를 눌러주세요..!! <br>
		<hr color=blue>
		<br>
			...
			<tbody width="90%" border="2" align="center">
				<tr>
					<c:forEach var="vo" items="${list}">
						...
						<!-- onclick으로 submit 하기 -->
						<td>${vo.cart_id}</td>
						<td style="width: 90pt;">${user_Id}</td>
						<td>${vo.product_id}</td>
			...
			<tbody width="90%" border="3" align="center">
				<tr>
					<c:forEach var="vo2" items="${list2}">
						<!-- for:Each : 반복문 -->
						<td>${vo2.mini_name}</td>
						<td>${vo2.mini_content}<br> <input type="button" onclick="window.open('miniimg.jsp?mini_nm=${vo2.mini_nm}','name',' width=850 height=800');" value=">>자세히"></td>
						<!-- window.open을 이용하여 새로운 창을 뜨게함  -->
						<td><img src="${vo2.mini_image}"></td>
						<td>${vo2.mini_price}</td>
						<td><a href="add?cart_image=${vo2.mini_image} &product_id=${vo2.mini_name} &cart_price=${vo2.mini_price} &user_id=${user_Id} &cart_count=1">담기 ➔</a></td>
						<tr>
					</c:forEach>
				</tr>
			</tbody>
